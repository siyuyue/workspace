/* ------------------------------------------------------------
    Name:     Jacobi Iteration with MPI N*1, Approach 1
    Author:   Siyu Yue
    Date:     11/10/2013
------------------------------------------------------------ */
#include <stdio.h>
...
int main(int argc, char **argv)
{
    ...
    for(k=0;k<STEP;k++)
    {
		...
		if( rank%2 == 0 )
		{
			if( rank != 0 )
			{
				MPI_Sendrecv(matB,N,MPI_DOUBLE,rank-1,0,leftbuf,N,MPI_DOUBLE,rank-1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			}
			if( rank != size - 1 )
			{
				MPI_Sendrecv(matB+(M-1)*N,N,MPI_DOUBLE,rank+1,0,rightbuf,N,MPI_DOUBLE,rank+1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			}
		}
		else
		{
...
#ifdef OUTPUT
	MPI_File_open(MPI_COMM_WORLD,"out.txt",MPI_MODE_WRONLY|MPI_MODE_CREATE,MPI_INFO_NULL,&fout);
	for(i=0;i<N;i++)
    {
		if( rank != 0 )
